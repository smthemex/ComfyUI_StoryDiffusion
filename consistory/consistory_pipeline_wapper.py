    ...
    def __call__(
        ...
        **kwargs,
        ...
        if isinstance(prompt, list):
            p_emb,po_emb=[],[]
            for i in prompt:
                tokens_p = cf_clip.tokenize(i)
                output_p = cf_clip.encode_from_tokens(tokens_p, return_dict=True)  # {"pooled_output":tensor}
                prompt_embeds_,pooled_prompt_embeds_ = output_p.pop("cond").to(device,self.unet.dtype),output_p.pop("pooled_output").to(device,self.unet.dtype)
                p_emb.append(prompt_embeds_),
                po_emb.append(pooled_prompt_embeds_)
            prompt_embeds = torch.cat(p_emb,dim=0)
            pooled_prompt_embeds = torch.cat(po_emb,dim=0)
        else:
            tokens_p = cf_clip.tokenize(prompt)
            output_p = cf_clip.encode_from_tokens(tokens_p, return_dict=True)  # {"pooled_output":tensor}
            prompt_embeds,pooled_prompt_embeds = output_p.pop("cond").to(device,self.unet.dtype),output_p.pop("pooled_output").to(device,self.unet.dtype)
        
        if negative_prompt is not None:
            tokens_n = cf_clip.tokenize(negative_prompt)
            output_n = cf_clip.encode_from_tokens(tokens_n, return_dict=True)  # {"pooled_output":tensor}
            negative_prompt_embeds,negative_pooled_prompt_embeds = output_n.pop("cond").to(device,self.unet.dtype),output_n.pop("pooled_output").to(device,self.unet.dtype)
            if isinstance(prompt, list):
                negative_prompt_embeds = torch.cat(batch_size*[negative_prompt_embeds] ,dim=0)
                negative_pooled_prompt_embeds = torch.cat(batch_size*[negative_pooled_prompt_embeds] ,dim=0)
        else:
            negative_prompt_embeds = torch.zeros_like(prompt_embeds)
            negative_pooled_prompt_embeds = torch.zeros_like(pooled_prompt_embeds)
        
        # print("prompt_embeds",prompt_embeds.shape) #prompt_embeds torch.Size([2, 77, 2048])
        # print("prompt_embeds",prompt_embeds.shape) #prompt_embeds torch.Size([2, 77, 2048])
        # print("negative_prompt_embeds",negative_prompt_embeds.shape)#negative_prompt_embeds torch.Size([1, 77, 2048])
        # print("pooled_prompt_embeds",pooled_prompt_embeds.shape)#pooled_prompt_embeds torch.Size([2, 1280])
        # print("negative_pooled_prompt_embeds",negative_pooled_prompt_embeds.shape)#negative_pooled_prompt_embeds torch.Size([1, 1280])
       
        
        
        # if prompt is not None: #PASS it
        #     (
        #         prompt_embeds,
        #         negative_prompt_embeds,
        #         pooled_prompt_embeds,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if share_queries and (i >= query_store.t_range[0] and i <= query_store.t_range[1]):
                    ...
                    # print(latent_model_input.shape) #[4, 4, 128, 128]
                    # print(prompt_embeds.shape)#([4, 77, 2048])
        ...
        if not output_type == "latent":
            ...
            #print(self.vae.config.force_upcast)
            needs_upcasting = self.vae.dtype == torch.float16 and self.vae.config.force_upcast
            #print(needs_upcasting,112233)
            if needs_upcasting:
                self.upcast_vae()
