...
class StableDiffusionXLStoryMakerPipeline(StableDiffusionXLPipeline):
    ...
    def load_storymaker_adapter(self, model_ckpt, image_emb_dim=512, num_tokens=20, scale=0.8, lora_scale=0.8,controlnet=None):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if prompt is not None:  #直接用clip的emb
            (
                prompt_embeds,
        ...
        if prompt_image_emb is  None:
            prompt_image_emb,control_image = self._encode_prompt_image_emb(image, image_2, mask_image, mask_image_2, face_info, face_info_2, cloth,cloth_2,
                                                         device, num_images_per_prompt,
                                                         self.unet.dtype, self.do_classifier_free_guidance)
        else:
            id_embeds, clip_image_embeds, clip_face_embeds=prompt_image_emb[0],prompt_image_emb[1],prompt_image_emb[2]
            ...
            #prompt_image_emb = prompt_image_emb.repeat(1, num_images_per_prompt, 1)
            prompt_image_emb = prompt_image_emb.view(bs_embed * num_images_per_prompt, seq_len, -1)
            prompt_image_emb.to(device=device, dtype=self.unet.dtype)

        control_net=self.controlnet.to(device) if self.controlnet is not None else None
        if isinstance(control_image,torch.Tensor) and control_net is not None:
            control_mode = True
        if control_mode:
        ...
        encoder_hidden_states = torch.cat([prompt_embeds, prompt_image_emb], dim=1).to(dtype=self.unet.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if control_mode:
                    ...
                    # predict the noise residual
                    #print(latent_model_input.shape,latent_model_input.dtype, encoder_hidden_states.shape, encoder_hidden_states.dtype) #torch.Size([2, 4, 64, 64]) torch.float16 torch.Size([2, 117, 2048]) torch.float16
                    noise_pred = self.unet(
                        latent_model_input,
                        t,
                        encoder_hidden_states=encoder_hidden_states,
...
CLIPImageProcessor_= CLIPImageProcessor()
...
def prepare_image_(
    ...
    #     vae_scale_factor=self.vae_scale_factor, do_convert_rgb=True, do_normalize=False
    # )
    img=torch.from_numpy(np.array(image).astype(np.float32) / 255.0).unsqueeze(0) 
    print (img.shape)
    image= vae.encode(img[:,:,:,:3]).to(dtype=torch.float32)
    #image = self.control_image_processor.preprocess(image, height=height, width=width).to(dtype=torch.float32)
    image_batch_size = image.shape[0]
...
def crop_image_( ori_img, ori_mask, face_info):
    ori_img = np.array(ori_img)
    ori_mask = np.array(ori_mask)
    crop, mask = bounding_rectangle(ori_img, ori_mask)
    mask = cv2.GaussianBlur(mask, (5, 5), 0)/255.
    ...
    # face_image = face_align.norm_crop(crop, landmark=face_kps.numpy(), image_size=224)  #  224
    face_image = face_align.norm_crop(ori_img, landmark=face_kps, image_size=224)  #  224
    clip_face = CLIPImageProcessor_(images=face_image, return_tensors="pt").pixel_values
    
    ref_img = Image.fromarray(crop)
    ref_img = ref_img.resize((224, 224))  
    clip_img = CLIPImageProcessor_(images=ref_img, return_tensors="pt").pixel_values
    return clip_img, clip_face, torch.from_numpy(face_info.normed_embedding).unsqueeze(0)

def encode_prompt_image_emb_( image, image_2, mask_image, mask_image_2, face_info, face_info_2, cloth, cloth_2,device, num_images_per_prompt, dtype, image_encoder,vae,do_classifier_free_guidance=True):
    crop_list = []; face_list = [];  id_list = []
    cn_img=None
    control_image=None
    if isinstance(image,list) :
    ...
    if cn_img is not None:
        ...
        # timestamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        # cn_img.save(f"{timestamp}.png")
        control_image = prepare_image_(vae,
            image=cn_img,
            width=width,
            height=height,
            batch_size=1 * num_images_per_prompt,
    ...
    if image_0 is not None:
        clip_img, clip_face, face_emb = crop_image_(image_0, mask_image, face_info)
        crop_list.append(clip_img)
        face_list.append(clip_face)
        id_list.append(face_emb)
    if image_2 is not None:
        clip_img, clip_face, face_emb = crop_image_(image_2, mask_image_2, face_info_2)
        crop_list.append(clip_img)
        face_list.append(clip_face)
        id_list.append(face_emb)
    if cloth is not None:
        crop_list = []
        clip_img = CLIPImageProcessor_(images=cloth.resize((224, 224)), return_tensors="pt").pixel_values
        crop_list.append(clip_img)
    if cloth_2 is not None:
        clip_img = CLIPImageProcessor_(images=cloth_2.resize((224, 224)), return_tensors="pt").pixel_values
        crop_list.append(clip_img)
    assert len(crop_list)>0, f"input error, images is None"
    
    clip_image = torch.cat(crop_list, dim=0).to(device, dtype=dtype)
    #clip_image_embeds = self.image_encoder(clip_image, output_hidden_states=True).hidden_states[-2]
    clip_image_embeds= image_encoder.model(pixel_values=clip_image, intermediate_output=-2)[1].to(device)

    clip_face = torch.cat(face_list, dim=0).to(device, dtype=dtype)
    clip_face_embeds= image_encoder.model(pixel_values= clip_face, intermediate_output=-2)[1].to(device)
    #clip_face_embeds = self.image_encoder(clip_face, output_hidden_states=True).hidden_states[-2]
    ...
    return (id_embeds, clip_image_embeds, clip_face_embeds),control_image