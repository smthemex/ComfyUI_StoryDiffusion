...
class QwenImageEditPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def _encode_vae_image(self, image: torch.Tensor, generator: torch.Generator):
        if isinstance(generator, list):
            ...
            image=image.to(self.vae.device,self.vae.dtype)
    ...
    def prepare_latents(
        ...
        if image_latents is None:
            ...
            latents_std = (
                torch.tensor(self.vae.latents_std)
                .view(1, self.latent_channels, 1, 1, 1)
                .to(image_latents.device, image_latents.dtype)
            )
            image_latents = (image_latents - latents_mean) / latents_std
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #print(latents.dtype,latents.device,image_latents.dtype,image_latents.device ) #torch.float32,cuda  torch.float32 cpu
                latent_model_input = latents
                if image_latents is not None:
                    image_latents= image_latents.to(latents.device)
                    latent_model_input = torch.cat([latents, image_latents], dim=1)
                latent_model_input= latent_model_input.to(self.transformer.device, dtype=self.transformer.dtype)
                # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
                timestep = t.expand(latents.shape[0]).to(latents.dtype)
                with self.transformer.cache_context("cond"):
                    #print(latent_model_input.dtype,prompt_embeds_mask.dtype,prompt_embeds.dtype) #torch.bfloat16 torch.int64 torch.float32
