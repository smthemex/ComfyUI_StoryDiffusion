...
class PhotoMakerStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def load_photomaker_adapter(
        ...
        **kwargs,
        ...
        self.tokenizer_2=self.tokenizer_2
    ...
    def encode_prompt_with_trigger_word(
        ...
        print(f"text_encoder_2 device: {self.text_encoder_2.device}")  # 检查设备是否为 meta # cpu
        print(f"text_encoder_2 param is meta: {next(self.text_encoder_2.parameters()).is_meta}") #False
        ...
        print(prompt_embeds.shape,1234) #torch.Size([1, 77, 2048]) 
        print(pooled_prompt_embeds.shape,pooled_prompt_embeds)#([1, 1280])
        prompt_embeds = prompt_embeds.to(device=device,dtype=self.text_encoder_2.dtype, )
    ...
    def __call__(
        ...
        if self.text_encoder_2 is None:
            text_encoder_projection_dim = int(pooled_prompt_embeds_text_only_arr[0].shape[-1])
        else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i <= start_merge_step or nc_flag:  
                    ...
                    negative_prompt_embeds=negative_prompt_embeds.to(latent_model_input.device,dtype=self.unet.dtype)
                    prompt_embeds=prompt_embeds.to(latent_model_input.device,dtype=self.unet.dtype)
                    negative_pooled_prompt_embeds=negative_pooled_prompt_embeds.to(latent_model_input.device,dtype=self.unet.dtype)
                    pooled_prompt_embeds=pooled_prompt_embeds.to(latent_model_input.device,dtype=self.unet.dtype)
