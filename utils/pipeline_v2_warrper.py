...
class PhotoMakerStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def load_photomaker_adapter(
        ...
        **kwargs,
        ...
        # id_encoderv1 = id_encoderv1.to("cuda", dtype=self.unet.dtype)
        # self.id_encoderv1=id_encoderv1
    ...
    def encode_prompt_with_trigger_word(
        ...
        if prompt is not None and isinstance(prompt, str):  #story输入的prompt是单条的
            batch_size = 1
        ...
        if prompt_embeds is None:
            ...
            #prompt_2 = [prompt_2] if isinstance(prompt_2, str) else prompt_2  #没有用到prompt2
    ...
    def __call__(
        ...
        **kwargs,
        ...
        negative_prompt_embeds_arr=emb_dict['negative_prompt_embeds_arr']
        prompt_embeds_arr=emb_dict['prompt_embeds_arr']
        pooled_prompt_embeds_arr=emb_dict['pooled_prompt_embeds_arr']
        prompt_embeds_text_only_arr=emb_dict['prompt_embeds_text_only_arr']
        pooled_prompt_embeds_text_only_arr=emb_dict['pooled_prompt_embeds_text_only_arr']
        negative_pooled_prompt_embeds_arr=emb_dict['negative_pooled_prompt_embeds_arr']
        negative_prompt_embeds=emb_dict['negative_pooled_prompt_embeds']
        pooled_prompt_embeds=emb_dict['pooled_prompt_embeds']    
        negative_pooled_prompt_embeds=emb_dict['negative_pooled_prompt_embeds']
        ...
        if prompt is not None and isinstance(prompt, str):
            ...
            batch_size =  emb_dict["batch_size"]
        ...
        latents = self.prepare_latents(
            ...
            height,
            width,
            prompt_embeds.dtype if not nc_flag else prompt_embeds_text_only.dtype, #nc时为none
            device,
            generator,
            latents,
        )
        ...
        if self.text_encoder_2 is None:
            text_encoder_projection_dim = int(pooled_prompt_embeds_text_only_arr[0].shape[-1])# 768
        else:
        ...
        #add_time_ids = torch.cat([add_time_ids, add_time_ids], dim=0)# 多加了一维v1版？
        add_time_ids = add_time_ids.to(device).repeat(batch_size * num_images_per_prompt, 1)
        
        # if ip_adapter_image is not None or ip_adapter_image_embeds is not None: #不需要
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # if i <= start_merge_step or nc_flag:  #忽略do_classifier_free_guidance
                ...
                if i <= start_merge_step or nc_flag:
                    negative_prompt_embeds=negative_prompt_embeds.to(self.unet.device,dtype=self.unet.dtype)
                    prompt_embeds_text_only=prompt_embeds_text_only.to(self.unet.device,dtype=self.unet.dtype)
                    pooled_prompt_embeds_text_only=pooled_prompt_embeds_text_only.to(self.unet.device,dtype=self.unet.dtype)
                    negative_pooled_prompt_embeds=negative_pooled_prompt_embeds.to(self.unet.device,dtype=self.unet.dtype)
                    current_prompt_embeds = torch.cat(
                        [negative_prompt_embeds, prompt_embeds_text_only], dim=0
                    )
                    add_text_embeds = torch.cat(
                    ...
                    pooled_prompt_embeds=pooled_prompt_embeds.to(self.unet.device,dtype=self.unet.dtype)
                    negative_pooled_prompt_embeds=negative_pooled_prompt_embeds.to(self.unet.device,dtype=self.unet.dtype)
                    current_prompt_embeds = torch.cat(
                        [negative_prompt_embeds, prompt_embeds], dim=0
                    )
                    add_text_embeds = torch.cat([negative_pooled_prompt_embeds, pooled_prompt_embeds],
                                                dim=0)
                add_text_embeds = add_text_embeds.to(latent_model_input.device,dtype=self.unet.dtype)
                add_time_ids=   add_time_ids.to(latent_model_input.device,dtype=self.unet.dtype)
                added_cond_kwargs = {"text_embeds": add_text_embeds, "time_ids": add_time_ids}
                # if ip_adapter_image is not None or ip_adapter_image_embeds is not None:
                #     added_cond_kwargs["image_embeds"] = image_embeds
                #
                # predict the noise residual
                current_prompt_embeds=current_prompt_embeds.to(latent_model_input.device,dtype=self.unet.dtype)
                #print(latent_model_input.dtype, current_prompt_embeds.dtype, t.dtype, added_cond_kwargs["text_embeds"].dtype, added_cond_kwargs["time_ids"].dtype)
                ...
                if callback_on_step_end is not None:
                    ...
                    negative_add_time_ids = callback_outputs.pop("negative_add_time_ids",
        ...
        if not output_type == "latent":
            ...
            has_latents_mean = hasattr(self.vae.config,
                                       "latents_mean") and self.vae.config.latents_mean is not None
            has_latents_std = hasattr(self.vae.config,
                                      "latents_std") and self.vae.config.latents_std is not None
            if has_latents_mean and has_latents_std:
                latents_mean = (
                    torch.tensor(self.vae.config.latents_mean).view(1, 4, 1, 1).to(latents.device,
                                                                                   latents.dtype)
                )
                latents_std = (
                    torch.tensor(self.vae.config.latents_std).view(1, 4, 1, 1).to(latents.device,
                                                                                  latents.dtype)
                )
